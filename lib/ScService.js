...
class WindowCovering extends homebridgeLib.ServiceDelegate {
  constructor (accessory, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      unit: '%',
      value: 100, // % open
      props: { minValue: 0, maxValue: 100, minStep: this.supportsUp ? 10 : 5 }
  ...
  setShadePosition () {
    ...
    this.timer = setTimeout(async () => {
      try {
        ...
        if (this.supportsUp && this.values.closeUpwards) {
          position *= -1
        }
  ...
  updatePosition (position) {
    ...
    // code below happens too soon.
    if (this.timer == null) {
      if (this.supportsUp && this.values.position !== 0) {
