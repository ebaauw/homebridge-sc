...
class WindowCovering extends homebridgeLib.ServiceDelegate {
  ...
  setShadePosition () {
    ...
    this.timer = setTimeout(async () => {
      try {
        let pos = 100 - this.values.targetPosition
        ...
        this.values.positionState = pos < this.pos
          ? this.Characteristics.hap.PositionState.INCREASING
          : this.Characteristics.hap.PositionState.DECREASING
        await this.client.setShadePosition(this.id, pos, this.values.morningMode)
        this.pos = pos
  ...
  updatePosition (pos) {
    this.pos = pos
    this.values.closeUpwards = pos < 0
    if (pos < 0) {
      pos *= -1
    }
    pos = 100 - pos
    this.values.currentPosition = pos
    this.values.targetPosition = pos
